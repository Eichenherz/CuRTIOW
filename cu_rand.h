...
__forceinline__ __host__ __device__ float RandFloat( u32 idx, u32 seed = 0 )
{
    // NOTE: Fast float generation from masked bits
    return ( float ) ( RandInt( idx, seed ) & 0x00FFFFFF ) / ( float ) ( 0x01000000 );
}
__forceinline__ __host__ __device__ float2 RandVec2( u32 idx, u32 seed = 0 )
{
    return { RandFloat( idx, seed ), RandFloat( idx + PRIME1, seed ) };
}
__forceinline__ __host__ __device__ float3 RandVec3( u32 idx, u32 seed = 0 )
{
    return { RandFloat( idx + PRIME1, seed ), RandFloat( idx + PRIME2, seed ), RandFloat( idx + PRIME3, seed ) };
}
